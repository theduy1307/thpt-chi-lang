            ...
            try
            {
                ...
                var _item = _context.ViewNhanVien.Where(x => x.IdNv == id && x.Disable == 0).Join(_context.ViewAccount, emp => emp.IdNv, acc => acc.IdNv, (emp, acc) => new { emp, acc })
                                                                                      .Select(x => new IAccountInformation
                                                                                      {
                                                                                          ...
                                                                                          ManageClass = !x.emp.isStudent??false && x.emp.AllowCode.ToString().Contains("3") ? _context.Lop.Where(lop => lop.IdChuNhiem == x.emp.IdNv).OrderByDescending(lop=>lop.Id).Select(x => x.TenLop).FirstOrDefault() : "",
                                                                                          Department = !x.emp.isStudent??false && x.emp.IdMonHoc != null ? _context.MonHoc.Where(monHoc => monHoc.Id == x.emp.IdMonHoc).OrderByDescending(monHoc => monHoc.Id).Select(x => x.TenMonHoc).FirstOrDefault():"",
