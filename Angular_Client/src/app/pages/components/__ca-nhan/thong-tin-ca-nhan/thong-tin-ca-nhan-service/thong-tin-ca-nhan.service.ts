...
import { AccountInformation, PasswordChangeInformation } from "../thong-tin-ca-nhan-model/thong-tin-ca-nhan.model";

const API_ROOT_URL = environment.ApiRoot + "/account-information";

@Injectable({ providedIn: "root" })
export class ThongTinCaNhanService extends TableService<AccountInformation> implements OnDestroy {
  ...
  public fetch() {
    ...
    const request = this.find(this.tableState)
      .pipe(
        tap((res: TableResponseModel<AccountInformation>) => {
        ...
        finalize(() => {
          ...
          const itemIds = this.getItems.value.map((el: AccountInformation) => {
            const item = el as unknown as AccountInformation;
            return item.id;
          });
          this.patchStateWithoutFetch({
    ...
    return this.http
      ...
      .pipe(
        map((response) => {
          ...
          const result: TableResponseModel<AccountInformation> = {
            items: response.data,
  ...
  getItemById(id: number): Observable<any> {
    this.initCallService();
    return this.http.get(`${API_ROOT_URL}/employee/${id}`, { headers: this._httpHeaders }).pipe(
      catchError((err) => {
        this.setErrorMess(err);
        return of({});
      }),
  ...
  create(item: any): Observable<any> {
    this.initCallService();
    return this.http
      .post<AccountInformation>(API_ROOT_URL + "/_Insert", item, {
        headers: this._httpHeaders,
      })
      .pipe(
        catchError((err) => {
  ...
  update(item: any): Observable<any> {
    this.initCallService();
    return this.http
      .post<AccountInformation>(API_ROOT_URL + "/_Update", item, {
        headers: this._httpHeaders,
      })
      .pipe(
        catchError((err) => {
  ...
  changePassword(oldPassword: any, newPassword:any): Observable<any> {
    ...
    return this.http.post(`${API_ROOT_URL}/change-password`, value, { headers: this._httpHeaders }).pipe(
      catchError((err) => {
        this.setErrorMess(err);
        return of({});
      }),
