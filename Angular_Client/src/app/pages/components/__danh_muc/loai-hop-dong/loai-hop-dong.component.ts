...
import { GroupingState, ICreateAction, IDeleteAction, IDeleteSelectedAction, IEditAction, IGroupingView, ISearchView, ISortView, PaginatorState, SortState } from 'src/app/_metronic/shared/crud-table';
import { DeleteModalComponent } from '../../_common/_components/delete-modal/delete-modal.component';
import { LoaiHopDongAddComponent } from './loai-hop-dong-add/loai-hop-dong-add.component';
import { LoaiHopDongDetailComponent } from './loai-hop-dong-detail/loai-hop-dong-detail.component';
import { LoaiHopDongEditComponent } from './loai-hop-dong-edit/loai-hop-dong-edit.component';
...
export class LoaiHopDongComponent implements OnInit

{
  ...
  constructor(
    ...
    public services: LoaiHopDongService,    
  ) {
  ...
  searchForm() {
    ...
    const searchEventHD = this.searchGroup.controls.searchTermHD.valueChanges
      .pipe(
        debounceTime(150),
        distinctUntilChanged()
      )
      .subscribe((val) => this.searchHD(val));
    this.subscriptions.push(searchEvent);
    this.subscriptions.push(searchEventHD);
  }
  search(searchTerm: string) {
    this.services.patchState({ searchTerm });
  }

  searchHD(searchTermHD: string) {
    this.services.patchState({ searchTermHD });
  }
  ...
  create() {
    const modalRef = this.modalService.open(LoaiHopDongAddComponent, { size: 'xl' });
  ...
  edit(id: number) {
    const modalRef = this.modalService.open(LoaiHopDongEditComponent, { size: 'xl' });
  ...
  getDetail(id: number) {
    const modalRef = this.modalService.open(LoaiHopDongDetailComponent, { size: 'xl' });
    modalRef.componentInstance.id = id;
    modalRef.result.then(() =>
        this.services.fetch(),
        () => { }
